...
- 末尾のアフィリエイト定型文（「このブログの商品リンクから飛んで…」）の
  ブロックの上に区切り線 `---` を入れる。並び順は必ず：
  本文 →（任意で商品カード）→ `---` ではなく、
  **本文 → `---` → 商品カード → 定型文**（カードが無ければ 本文 → `---` → 定型文）。
  ＝区切り線は「定型文ブロックの上」。直前に商品カードがある場合は
  そのカードの上に置く（カードと定型文の間には入れない）。
  実例: amaike-yakiniku-2026 / frieren-anime-manga-review。
  （`---` の入れ忘れが起きやすいので投稿時・委任指示時に必ず確認）
